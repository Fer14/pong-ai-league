...
from sarsa_player import SARSAPaddle
import pygame
...
class SARSAAgent:
    ...
    def train(self, terminal_state):
        ...
        for index, (
            ...
            if not done:
                new_qs = self.model.predict(np.array([new_current_state]), verbose=0)
                next_action = np.argmax(new_qs)
                # new_q = reward + DISCOUNT * future_qs_list[0][next_action]
                new_q = reward + DISCOUNT * new_qs[0][next_action]
            else:
                new_q = reward
...
class PongGameSARSATraining(PongGame):
    def __init__(
...
def main():
    ...
    agent = SARSAAgent()
    env = PongGameSARSATraining(default_pong=False, display=DISPLAY)
    ...
    for episode in tqdm(range(1, EPISODES + 1), unit="episodes"):
        ...
        current_state = env.restart_sarsa()
